...
RegisterCommand('twt', function(playerId, args, rawCommand)
	if playerId == 0 then
		print('[^1ERROR^7] Este comando no se puede usar mediente consola!')
	else
...
RegisterCommand('msg', function(source, args, user)
	...
	if GetPlayerName(tonumber(args[1])) then
		...
		TriggerClientEvent('chat:addMessage', player, {args = {"^1PM de "..GetPlayerName(source).. "[" .. source .. "]: ^7" ..table.concat(args, " ")}, color = {255, 153, 0}})
		TriggerClientEvent('chat:addMessage', source, {args = {"^1PM ENVIADO A "..GetPlayerName(player).. "[" .. player .. "]: ^7" ..table.concat(args, " ")}, color = {255, 153, 0}})
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Ese jugador no existe!")
	end
